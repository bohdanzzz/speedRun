...
const analyzeResponse = (gamesFromResponse, prevGames, startTS, nick, timeClass) => {
    return gamesFromResponse.filter((g) => {
...
const getDurationFromPGN = (pgnData) => {
    try {
        ...
        return Math.round(tEnd.getTime() / 1000) - Math.round(tStart.getTime() / 1000);
    } catch (e) {
...
const parseOldGames = async (nick, startTS, timeClass) => {
    ...
    for (year = startYear; year <= currentYear; year += 1) {
        for (month = ((startYear === year) ? startMonth : 1); month <= ((year === currentYear) ? currentMonth : 12); month += 1) {
            let response;
            try {
                response = await axios.get(`https://api.chess.com/pub/player/${nick}/games/${year}/${month < 10 ? '0' + month : month}`);
            } catch (error) {
                throw new Error(`Can not fetch games data: ${year}.${month}; ${error.message}`);
            }

            prevGames = analyzeResponse(response.data.games, JSON.parse(JSON.stringify(prevGames)), startTS, nick, timeClass);
        }
    }
...
new Vue({
    ...
    async mounted() {
        ...
        this.prevGames = await parseOldGames(this.nick, this.startTS, this.timeClass);
    ...
    computed: {
        href() {
            ...
            return `${currentLocation.origin}${currentLocation.pathname}?nick=${this.nick}&startTS=${parseInt(new Date(this.startDateTime) / 1000)}&timeClass=${this.timeClass}`;
        }
    },
